...
@implementation MCRecordEncoder
- (void)dealloc {
    _writer = nil;
    _videoInput = nil;
    _audioInput = nil;
...
+ (MCRecordEncoder *)encoderForPath:(NSString*) path
                             Height:(NSInteger) cy
                              width:(NSInteger) cx
                           channels: (int) ch
...
{
    MCRecordEncoder *enc = [MCRecordEncoder alloc];
    return [enc initPath:path Height:cy width:cx channels:ch samples:rate transform:transform];
}
               ...
               transform:(CGAffineTransform)transform
{
    ...
    if (self) {
        ...
        //初始化视频输出
        [self initVideoInputHeight:cy width:cx transform:transform];
        //确保采集到rate和ch
        if (rate != 0 && ch != 0) {
            //初始化音频输出
...
//初始化视频输入
- (void)initVideoInputHeight:(NSInteger)cy
                       width:(NSInteger)cx
                   transform:(CGAffineTransform)transform
{
