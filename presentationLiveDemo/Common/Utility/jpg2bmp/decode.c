...
static DWORD start_neg_pow2={0,-1,-3,-7,-15,-31,-63,-127,-255,-511,-1023,-2047,-4095,-8191,-16383,-32767};
...
int write_buf_to_BMP(BYTE *im_buffer, WORD X_bitmap, WORD Y_bitmap, char *BMPname)
{
    ...
    for (y=0;y<Y_bitmap;y++)
    {
        for (x=0;x<X_bitmap;x++)
        {
            ...
            //pixel.B=((im_loc_bytes)&0x000000FF);
            //pixel.G=((im_loc_bytes>>8)&0x000000FF);
            //pixel.R=((im_loc_bytes>>16)&0x000000FF);
...
int jpg2bmp(char *jpgPath,char *bmpPath)
{
    ...
    if (!load_JPEG_header(fp,&X_image,&Y_image)) {exitmessage(error_string);return -1;}
    fclose(fp);
    ...
    if (!get_JPEG_buffer(X_image,Y_image,&our_image_buffer)) {exitmessage(error_string);return -1;}
    int size=write_buf_to_BMP(our_image_buffer,X_image,Y_image, bmpPath);
    return size;
}
