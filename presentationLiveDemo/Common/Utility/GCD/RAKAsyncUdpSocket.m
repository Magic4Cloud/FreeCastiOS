...
@class RAKAsyncUdpSendPacket;

NSString *const RAKAsyncUdpSocketException = @"RAKAsyncUdpSocketException";
NSString *const RAKAsyncUdpSocketErrorDomain = @"RAKAsyncUdpSocketErrorDomain";

NSString *const RAKAsyncUdpSocketQueueName = @"RAKAsyncUdpSocket";
NSString *const RAKAsyncUdpSocketThreadName = @"RAKAsyncUdpSocket-CFStream";

enum RAKAsyncUdpSocketFlags
{
...
@interface RAKAsyncUdpSocket ()
{
	...
	RAKAsyncUdpSocketReceiveFilterBlock receiveFilterBlock;
	dispatch_queue_t receiveFilterQueue;
	BOOL receiveFilterAsync;
	
	RAKAsyncUdpSocketSendFilterBlock sendFilterBlock;
	...
	RAKAsyncUdpSendPacket *currentSend;
...
/**
 * The RAKAsyncUdpSendPacket encompasses the instructions for a single send/write.
**/
@interface RAKAsyncUdpSendPacket : NSObject {
@public
	NSData *buffer;
...
@implementation RAKAsyncUdpSendPacket
...
@interface RAKAsyncUdpSpecialPacket : NSObject {
@public
//	uint8_t type;
...
@implementation RAKAsyncUdpSpecialPacket
...
@implementation RAKAsyncUdpSocket
...
- (id)initWithDelegate:(id)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	...
	if ((self = [super init]))
	{
		...
		{
			socketQueue = dispatch_queue_create([RAKAsyncUdpSocketQueueName UTF8String], NULL);
		}
...
- (NSError *)badConfigError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:RAKAsyncUdpSocketErrorDomain
	                           code:RAKAsyncUdpSocketBadConfigError
	                       userInfo:userInfo];
}
...
- (NSError *)badParamError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:RAKAsyncUdpSocketErrorDomain
	                           code:RAKAsyncUdpSocketBadParamError
	                       userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"RAKAsyncUdpSocketSendTimeoutError",
	                                                     @"RAKAsyncUdpSocket", [NSBundle mainBundle],
	                                                     @"Send operation timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:RAKAsyncUdpSocketErrorDomain
	                           code:RAKAsyncUdpSocketSendTimeoutError
	                       userInfo:userInfo];
}

- (NSError *)socketClosedError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"RAKAsyncUdpSocketClosedError",
	                                                     @"RAKAsyncUdpSocket", [NSBundle mainBundle],
	                                                     @"Socket closed", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:RAKAsyncUdpSocketErrorDomain code:RAKAsyncUdpSocketClosedError userInfo:userInfo];
}
...
- (NSError *)otherError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:RAKAsyncUdpSocketErrorDomain
	                           code:RAKAsyncUdpSocketOtherError
	                       userInfo:userInfo];
}
...
- (BOOL)connectToHost:(NSString *)host onPort:(uint16_t)port error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		RAKAsyncUdpSpecialPacket *packet = [[RAKAsyncUdpSpecialPacket alloc] init];
...
- (void)maybeConnect
{
	...
	BOOL sendQueueReady = [currentSend isKindOfClass:[RAKAsyncUdpSpecialPacket class]];
	
	if (sendQueueReady)
	{
		RAKAsyncUdpSpecialPacket *connectPacket = (RAKAsyncUdpSpecialPacket *)currentSend;
...
- (void)sendData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	RAKAsyncUdpSendPacket *packet = [[RAKAsyncUdpSendPacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (void)sendData:(NSData *)data toAddress:(NSData *)remoteAddr withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	RAKAsyncUdpSendPacket *packet = [[RAKAsyncUdpSendPacket alloc] initWithData:data timeout:timeout tag:tag];
	packet->addressFamily = [RAKAsyncUdpSocket familyFromAddress:remoteAddr];
...
{
	RAKAsyncUdpSocketSendFilterBlock newFilterBlock = NULL;
...
- (void)maybeDequeueSend
{
	...
	if (currentSend == nil)
	{
		...
		while ([sendQueue count] > 0)
		{
			...
			if ([currentSend isKindOfClass:[RAKAsyncUdpSpecialPacket class]])
			{
				[self maybeConnect];
...
- (void)doPreSend
{
	...
	if (sendFilterBlock && sendFilterQueue)
	{
		...
		if (sendFilterAsync)
		{
			...
			RAKAsyncUdpSendPacket *sendPacket = currentSend;
...
- (void)setReceiveFilter:(RAKAsyncUdpSocketReceiveFilterBlock)filterBlock
          ...
          isAsynchronous:(BOOL)isAsynchronous
{
	RAKAsyncUdpSocketReceiveFilterBlock newFilterBlock = NULL;
	dispatch_queue_t newFilterQueue = NULL;
...
+ (void)listenerThread
{
	@autoreleasepool {
		...
		[[NSThread currentThread] setName:RAKAsyncUdpSocketThreadName];
...
+ (void)addStreamListener:(RAKAsyncUdpSocket *)asyncUdpSocket
{
	LogTrace();
...
+ (void)removeStreamListener:(RAKAsyncUdpSocket *)asyncUdpSocket
{
	LogTrace();
	NSAssert([NSThread currentThread] == listenerThread, @"Invoked on wrong thread");
...
{
	@autoreleasepool {
		RAKAsyncUdpSocket *asyncUdpSocket = (__bridge RAKAsyncUdpSocket *)pInfo;
