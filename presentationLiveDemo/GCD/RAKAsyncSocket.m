...
#ifndef RAKAsyncSocketLoggingEnabled
#define RAKAsyncSocketLoggingEnabled 0
#endif

#if RAKAsyncSocketLoggingEnabled

// Logging Enabled - See log level below

// Logging uses the CocoaLumberjack framework (which is also RAK based).
...
#define LogAsync   YES
#define LogContext RAKAsyncSocketLoggingContext
...
#ifndef RAKAsyncSocketLogLevel
#define RAKAsyncSocketLogLevel LOG_LEVEL_VERBOSE
#endif

// Log levels : off, error, warn, info, verbose
static const int logLevel = RAKAsyncSocketLogLevel;
...
NSString *const RAKAsyncSocketException = @"RAKAsyncSocketException";
NSString *const RAKAsyncSocketErrorDomain = @"RAKAsyncSocketErrorDomain";

NSString *const RAKAsyncSocketQueueName = @"RAKAsyncSocket";
NSString *const RAKAsyncSocketThreadName = @"RAKAsyncSocket-CFStream";

NSString *const RAKAsyncSocketManuallyEvaluateTrust = @"RAKAsyncSocketManuallyEvaluateTrust";
#if TARGET_OS_IPHONE
NSString *const RAKAsyncSocketUseCFStreamForTLS = @"RAKAsyncSocketUseCFStreamForTLS";
#endif
NSString *const RAKAsyncSocketSSLPeerID = @"RAKAsyncSocketSSLPeerID";
NSString *const RAKAsyncSocketSSLProtocolVersionMin = @"RAKAsyncSocketSSLProtocolVersionMin";
NSString *const RAKAsyncSocketSSLProtocolVersionMax = @"RAKAsyncSocketSSLProtocolVersionMax";
NSString *const RAKAsyncSocketSSLSessionOptionFalseStart = @"RAKAsyncSocketSSLSessionOptionFalseStart";
NSString *const RAKAsyncSocketSSLSessionOptionSendOneByteRecord = @"RAKAsyncSocketSSLSessionOptionSendOneByteRecord";
NSString *const RAKAsyncSocketSSLCipherSuites = @"RAKAsyncSocketSSLCipherSuites";
#if !TARGET_OS_IPHONE
NSString *const RAKAsyncSocketSSLDiffieHellmanParameters = @"RAKAsyncSocketSSLDiffieHellmanParameters";
#endif

enum RAKAsyncSocketFlags
{
...
@interface RAKAsyncSocketPreBuffer : NSObject
{
	uint8_t *preBuffer;
...
@implementation RAKAsyncSocketPreBuffer
...
/**
 * The RAKAsyncReadPacket encompasses the instructions for any given read.
 * The content of a read packet allows the code to determine if we're:
 *  - reading to a certain length
 *  - reading to a certain separator
 *  - or simply reading the first chunk of available data
**/
@interface RAKAsyncReadPacket : NSObject
{
  @public
...
- (NSUInteger)readLengthForTermWithPreBuffer:(RAKAsyncSocketPreBuffer *)preBuffer found:(BOOL *)foundPtr;

- (NSInteger)searchForTermAfterPreBuffering:(ssize_t)numBytes;

@end

@implementation RAKAsyncReadPacket
...
/**
 * The RAKAsyncWritePacket encompasses the instructions for any given write.
**/
@interface RAKAsyncWritePacket : NSObject
{
  @public
	NSData *buffer;
	NSUInteger bytesDone;
...
@implementation RAKAsyncWritePacket
...
/**
 * The RAKAsyncSpecialPacket encompasses special instructions for interruptions in the read/write queues.
 * This class my be altered to support more than just TLS in the future.
**/
@interface RAKAsyncSpecialPacket : NSObject
{
  @public
...
@implementation RAKAsyncSpecialPacket
...
@implementation RAKAsyncSocket
{
	...
	RAKAsyncReadPacket *currentRead;
	RAKAsyncWritePacket *currentWrite;
	
	unsigned long socketFDBytesAvailable;
	
	RAKAsyncSocketPreBuffer *preBuffer;
	...
	RAKAsyncSocketPreBuffer *sslPreBuffer;
...
- (id)initWithDelegate:(id)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	if((self = [super init]))
	{
		...
		{
			socketQueue = dispatch_queue_create([RAKAsyncSocketQueueName UTF8String], NULL);
		}
		...
		preBuffer = [[RAKAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
	}
	return self;
}
...
- (BOOL)acceptOnInterface:(NSString *)inInterface port:(uint16_t)port error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		if (enableIPv4)
		{
			...
			__weak RAKAsyncSocket *weakSelf = self;
			
			dispatch_source_set_event_handler(accept4Source, ^{ @autoreleasepool {
			#pragma clang diagnostic push
			#pragma clang diagnostic warning "-Wimplicit-retain-self"
				
				__strong RAKAsyncSocket *strongSelf = weakSelf;
...
- (BOOL)doAccept:(int)parentSocketFD
{
	...
	if (delegateQueue)
	{
		...
		dispatch_async(delegateQueue, ^{ @autoreleasepool {
			...
			// Create RAKAsyncSocket instance for accepted socket
			
			RAKAsyncSocket *acceptedSocket = [[RAKAsyncSocket alloc] initWithDelegate:theDelegate
                ...
                error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		#pragma clang diagnostic warning "-Wimplicit-retain-self"
			...
			NSMutableArray *addresses = [RAKAsyncSocket lookupHost:hostCpy port:port error:&lookupErr];
			...
			else
			{
				...
				{
					if (!address4 && [RAKAsyncSocket isIPv4Address:address])
					{
						address4 = address;
					}
					else if (!address6 && [RAKAsyncSocket isIPv6Address:address])
					{
						address6 = address;
					}
				}
...
- (NSError *)badConfigError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:RAKAsyncSocketErrorDomain code:RAKAsyncSocketBadConfigError userInfo:userInfo];
}
...
- (NSError *)badParamError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:RAKAsyncSocketErrorDomain code:RAKAsyncSocketBadParamError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"RAKAsyncSocketConnectTimeoutError",
	                                                     @"RAKAsyncSocket", [NSBundle mainBundle],
	                                                     @"Attempt to connect to host timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:RAKAsyncSocketErrorDomain code:RAKAsyncSocketConnectTimeoutError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"RAKAsyncSocketReadMaxedOutError",
														 @"RAKAsyncSocket", [NSBundle mainBundle],
														 @"Read operation reached set maximum length", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:RAKAsyncSocketErrorDomain code:RAKAsyncSocketReadMaxedOutError userInfo:info];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"RAKAsyncSocketReadTimeoutError",
	                                                     @"RAKAsyncSocket", [NSBundle mainBundle],
	                                                     @"Read operation timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:RAKAsyncSocketErrorDomain code:RAKAsyncSocketReadTimeoutError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"RAKAsyncSocketWriteTimeoutError",
	                                                     @"RAKAsyncSocket", [NSBundle mainBundle],
	                                                     @"Write operation timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:RAKAsyncSocketErrorDomain code:RAKAsyncSocketWriteTimeoutError userInfo:userInfo];
}

- (NSError *)connectionClosedError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"RAKAsyncSocketClosedError",
	                                                     @"RAKAsyncSocket", [NSBundle mainBundle],
	                                                     @"Socket closed by remote peer", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:RAKAsyncSocketErrorDomain code:RAKAsyncSocketClosedError userInfo:userInfo];
}
...
- (NSError *)otherError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:RAKAsyncSocketErrorDomain code:RAKAsyncSocketOtherError userInfo:userInfo];
}
...
- (BOOL)usingCFStreamForTLS
{
	...
	{
		// The startTLS method was given the RAKAsyncSocketUseCFStreamForTLS flag.
                        ...
                        tag:(long)tag
{
	...
	RAKAsyncReadPacket *packet = [[RAKAsyncReadPacket alloc] initWithData:buffer
	                                                          startOffset:offset
	                                                            maxLength:length
	                                                              timeout:timeout
	                                                           readLength:0
...
- (float)progressOfReadReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
		...
		if (!currentRead || ![currentRead isKindOfClass:[RAKAsyncReadPacket class]])
		{
...
- (void)maybeDequeueRead
{
	...
	if ((currentRead == nil) && (flags & kConnected))
	{
		if ([readQueue count] > 0)
		{
			...
			if ([currentRead isKindOfClass:[RAKAsyncSpecialPacket class]])
			{
				LogVerbose(@"Dequeued RAKAsyncSpecialPacket");
			...
			{
				LogVerbose(@"Dequeued RAKAsyncReadPacket");
...
- (void)doReadData
{
	...
	if ([self usingCFStreamForTLS])
	{
		...
		// Requested CFStream, rather than SecureTransport, for TLS (via RAKAsyncSocketUseCFStreamForTLS)
...
- (void)completeCurrentRead
{
	...
	{
		RAKAsyncReadPacket *theRead = currentRead; // Ensure currentRead retained since result may not own buffer
...
- (void)doReadTimeout
{
	...
	{
		RAKAsyncReadPacket *theRead = currentRead;
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	RAKAsyncWritePacket *packet = [[RAKAsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (float)progressOfWriteReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
		...
		if (!currentWrite || ![currentWrite isKindOfClass:[RAKAsyncWritePacket class]])
		{
...
- (void)maybeDequeueWrite
{
	...
	if ((currentWrite == nil) && (flags & kConnected))
	{
		if ([writeQueue count] > 0)
		{
			...
			if ([currentWrite isKindOfClass:[RAKAsyncSpecialPacket class]])
			{
				LogVerbose(@"Dequeued RAKAsyncSpecialPacket");
			...
			{
				LogVerbose(@"Dequeued RAKAsyncWritePacket");
...
- (void)doWriteData
{
	...
	// Note: This method is not called if currentWrite is a RAKAsyncSpecialPacket (startTLS packet)
...
- (void)doWriteTimeout
{
	...
	{
		RAKAsyncWritePacket *theWrite = currentWrite;
...
- (void)startTLS:(NSDictionary *)tlsSettings
{
	...
	RAKAsyncSpecialPacket *packet = [[RAKAsyncSpecialPacket alloc] initWithTLSSettings:tlsSettings];
...
- (void)maybeStartTLS
{
	...
	if ((flags & kStartingReadTLS) && (flags & kStartingWriteTLS))
	{
		...
		#if TARGET_OS_IPHONE
		{
			RAKAsyncSpecialPacket *tlsPacket = (RAKAsyncSpecialPacket *)currentRead;
			NSDictionary *tlsSettings = tlsPacket->tlsSettings;
			
			NSNumber *value = [tlsSettings objectForKey:RAKAsyncSocketUseCFStreamForTLS];
...
{
	RAKAsyncSocket *asyncSocket = (__bridge RAKAsyncSocket *)connection;
...
- (void)ssl_startTLS
{
	...
	BOOL shouldManuallyEvaluateTrust = [[tlsSettings objectForKey:RAKAsyncSocketManuallyEvaluateTrust] boolValue];
	...
	//  3. RAKAsyncSocketSSLPeerID
	//  4. RAKAsyncSocketSSLProtocolVersionMin
	//  5. RAKAsyncSocketSSLProtocolVersionMax
	//  6. RAKAsyncSocketSSLSessionOptionFalseStart
	//  7. RAKAsyncSocketSSLSessionOptionSendOneByteRecord
	//  8. RAKAsyncSocketSSLCipherSuites
	//  9. RAKAsyncSocketSSLDiffieHellmanParameters (Mac)
	//
	...
	value = [tlsSettings objectForKey:RAKAsyncSocketSSLPeerID];
	...
	{
		NSAssert(NO, @"Invalid value for RAKAsyncSocketSSLPeerID. Value must be of type NSData."
		             @" (You can convert strings to data using a method like"
		             @" [string dataUsingEncoding:NSUTF8StringEncoding])");
		
		[self closeWithError:[self otherError:@"Invalid value for RAKAsyncSocketSSLPeerID."]];
		return;
	}
	
	// 4. RAKAsyncSocketSSLProtocolVersionMin
	
	value = [tlsSettings objectForKey:RAKAsyncSocketSSLProtocolVersionMin];
	...
	{
		NSAssert(NO, @"Invalid value for RAKAsyncSocketSSLProtocolVersionMin. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for RAKAsyncSocketSSLProtocolVersionMin."]];
		return;
	}
	
	// 5. RAKAsyncSocketSSLProtocolVersionMax
	
	value = [tlsSettings objectForKey:RAKAsyncSocketSSLProtocolVersionMax];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for RAKAsyncSocketSSLProtocolVersionMax. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for RAKAsyncSocketSSLProtocolVersionMax."]];
		return;
	}
	
	// 6. RAKAsyncSocketSSLSessionOptionFalseStart
	
	value = [tlsSettings objectForKey:RAKAsyncSocketSSLSessionOptionFalseStart];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for RAKAsyncSocketSSLSessionOptionFalseStart. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for RAKAsyncSocketSSLSessionOptionFalseStart."]];
		return;
	}
	
	// 7. RAKAsyncSocketSSLSessionOptionSendOneByteRecord
	
	value = [tlsSettings objectForKey:RAKAsyncSocketSSLSessionOptionSendOneByteRecord];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for RAKAsyncSocketSSLSessionOptionSendOneByteRecord."
		             @" Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for RAKAsyncSocketSSLSessionOptionSendOneByteRecord."]];
		return;
	}
	
	// 8. RAKAsyncSocketSSLCipherSuites
	
	value = [tlsSettings objectForKey:RAKAsyncSocketSSLCipherSuites];
	if ([value isKindOfClass:[NSArray class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for RAKAsyncSocketSSLCipherSuites. Value must be of type NSArray.");
		
		[self closeWithError:[self otherError:@"Invalid value for RAKAsyncSocketSSLCipherSuites."]];
		return;
	}
	
	// 9. RAKAsyncSocketSSLDiffieHellmanParameters
	
	#if !TARGET_OS_IPHONE
	value = [tlsSettings objectForKey:RAKAsyncSocketSSLDiffieHellmanParameters];
	if ([value isKindOfClass:[NSData class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for RAKAsyncSocketSSLDiffieHellmanParameters. Value must be of type NSData.");
		
		[self closeWithError:[self otherError:@"Invalid value for RAKAsyncSocketSSLDiffieHellmanParameters."]];
		return;
	}
	#endif
	...
	if (value)
	{
		NSAssert(NO, @"Security option unavailable - kCFStreamSSLLevel"
		             @" - You must use RAKAsyncSocketSSLProtocolVersionMin & RAKAsyncSocketSSLProtocolVersionMax");
	...
	sslPreBuffer = [[RAKAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
...
- (void)ssl_continueSSLHandshake
{
	...
	else if (status == errSSLPeerAuthCompleted)
	{
		...
		else
		{
			...
			NSString *msg = @"RAKAsyncSocketManuallyEvaluateTrust specified in tlsSettings,"
...
- (void)cf_startTLS
{
	...
	NSAssert([currentRead isKindOfClass:[RAKAsyncSpecialPacket class]], @"Invalid read packet for startTLS");
	NSAssert([currentWrite isKindOfClass:[RAKAsyncSpecialPacket class]], @"Invalid write packet for startTLS");
...
+ (void)startCFStreamThreadIfNeeded
{
	...
	dispatch_once(&predicate, ^{
		...
		cfstreamThreadSetupQueue = dispatch_queue_create("RAKAsyncSocket-CFStreamThreadSetup", DISPATCH_QUEUE_SERIAL);
	});
...
+ (void)stopCFStreamThreadIfNeeded
{
	...
	#pragma clang diagnostic warning "-Wimplicit-retain-self"
		...
		if (--cfstreamThreadRetainCount == 0)
		{
			...
			// wake up the thread
			[RAKAsyncSocket performSelector:@selector(ignore:)
...
{
	[[NSThread currentThread] setName:RAKAsyncSocketThreadName];
...
+ (void)scheduleCFStreams:(RAKAsyncSocket *)asyncSocket
{
	LogTrace();
...
+ (void)unscheduleCFStreams:(RAKAsyncSocket *)asyncSocket
{
	LogTrace();
	NSAssert([NSThread currentThread] == cfstreamThread, @"Invoked on wrong thread");
...
{
	RAKAsyncSocket *asyncSocket = (__bridge RAKAsyncSocket *)pInfo;
